...
describe('semantic pluralization (final coverage)', () => {
  ...
  test('fertility.days pluralization (EN/PL 1/2/5/12/22/25)', async () => {
    const enExp: Record<number, string> = {
      1: '1 day',
      2: '2 days',
      5: '5 days',
      12: '12 days',
    ...
    const plExp: Record<number, string> = {
      1: '1 dzień',
      2: '2 dni',
      5: '5 dni',
      12: '12 dni',
    ...
    for (const n of counts)
      expect(
        i18n.t('fertility.days', { defaultValue: '{{count}} days', count: n })
      ).toBe(enExp[n]);
  ...
  test('fertility.daysPastOvulation pluralization (no unit interpolation)', async () => {
    const enExp: Record<number, string> = {
      1: '1 day past ovulation',
      2: '2 days past ovulation',
      5: '5 days past ovulation',
    };
    const plExp: Record<number, string> = {
      1: '1 dzień po owulacji',
      2: '2 dni po owulacji',
      5: '5 dni po owulacji',
      12: '12 dni po owulacji',
      22: '22 dni po owulacji',
      25: '25 dni po owulacji',
    };
    await i18n.changeLanguage('en');
    for (const n of [1, 2, 5])
      expect(
        i18n.t('fertility.daysPastOvulation', {
  ...
  test('pregnancy.weekBanner.daysToGo (PL verb-inflected)', async () => {
    ...
    for (const n of counts)
      expect(
        i18n.t('pregnancy.weekBanner.daysToGo', {
          defaultValue: '{{days}} days to go',
          days: String(n),
          count: n,
        })
      ).toBe(plExp[n]);
  ...
  test('cycleCard.periodLate (PL, no raw day/days unit)', async () => {
    const plExp: Record<number, string> = {
      1: 'Miesiączka spóźnia się o 1 dzień',
      2: 'Miesiączka spóźnia się o 2 dni',
      5: 'Miesiączka spóźnia się o 5 dni',
      12: 'Miesiączka spóźnia się o 12 dni',
      22: 'Miesiączka spóźnia się o 22 dni',
      25: 'Miesiączka spóźnia się o 25 dni',
    };
    await i18n.changeLanguage('pl');
    for (const n of counts)
      expect(
        i18n.t('cycleCard.periodLate', {
  ...
  test('cycleCard.daysToDue (PL)', async () => {
    const plExp: Record<number, string> = {
      1: '1 dzień do terminu',
      2: '2 dni do terminu',
      5: '5 dni do terminu',
      12: '12 dni do terminu',
      22: '22 dni do terminu',
      25: '25 dni do terminu',
    };
    await i18n.changeLanguage('pl');
    for (const n of counts)
      expect(
        i18n.t('cycleCard.daysToDue', {
          defaultValue: '{{count}} days to due date',
          count: n,
        })
      ).toBe(plExp[n]);
  });
  ...
  test('workoutComplete.labels.allSets (PL seria/serie/serii)', async () => {
    ...
    await i18n.changeLanguage('pl');
    for (const n of [1, 2, 5, 12, 22, 25])
      expect(
        i18n.t('workoutComplete.labels.allSets', {
  ...
  test('cycleHub ring Day/cycle localized (EN/PL)', async () => {
    await i18n.changeLanguage('en');
    expect(
      i18n.t('cycleHub.ring.day', { defaultValue: 'Day {{day}}', day: 18 })
    ).toBe('Day 18');
    expect(
      i18n.t('cycleHub.ring.dayCycle', {
  ...
  test('medication cyclic schedule dual counts (PL)', async () => {
    ...
    for (const [on, off, expected] of cases) {
      const onText = i18n.t('medications.scheduleSummary.cycleOn', {
        defaultValue: '{{count}} days on',
        count: on,
      });
      const offText = i18n.t('medications.scheduleSummary.cycleOff', {
        defaultValue: '{{count}} days off',
        count: off,
      });
      const combined = i18n.t('medications.scheduleSummary.cycle', {
        defaultValue: '{{on}}, {{off}}',
        on: onText,
        off: offText,
      });
...
describe('baby development content (weeks 4-40)', () => {
  ...
  test('every week 4-40 has EN and PL comparison/baby/mom, PL != EN', async () => {
    ...
    const en: Record<
      number,
      { comparison: string; baby: string; mom: string }
    > = {};
    for (let w = 4; w <= 40; w++) {
  ...
  test('curated PL final copy for reviewed weeks', async () => {
    ...
    expect(w4.mom).toBe(
      'Miesiączka mogła się właśnie nie pojawić. Poziom hormonów ciążowych zaczyna rosnąć.'
    );
    const w15 = localizeBabyWeek(15, i18n.t)!;
    expect(w15.baby).toBe(
      'Dziecko wyczuwa światło, a jego ruchy wprawiają płyn owodniowy w ruch.'
    );
    const w20 = localizeBabyWeek(20, i18n.t)!;
    expect(w20.comparison).toBe('Banan');
    const w25 = localizeBabyWeek(25, i18n.t)!;
    expect(w25.baby).toBe(
      'Dziecku zaczynają rosnąć włosy, rozwija się odruch zaskoczenia.'
    );
    const w28 = localizeBabyWeek(28, i18n.t)!;
    expect(w28.baby).toBe(
      'Dziecko potrafi mrugać, a jego rzęsy są już w pełni wykształcone.'
    );
    const w37 = localizeBabyWeek(37, i18n.t)!;
    expect(w37.baby).toBe(
      'Dziecko jest już blisko terminu porodu (tzw. wczesny termin) i ćwiczy oddychanie.'
    );
  });
...
describe('pregnancy safety content', () => {
  ...
  test('canonical EN search resolves the exact controlled keys', async () => {
    const salmonKeys = lookupSafetyLocalized(
      'salmon',
      FOOD_SAFETY,
      'food',
      i18n.t
    ...
    const rawFishKeys = lookupSafetyLocalized(
      'raw fish',
      FOOD_SAFETY,
      'food',
      i18n.t
    ...
    const softCheeseKeys = lookupSafetyLocalized(
      'soft cheese',
      FOOD_SAFETY,
      'food',
      i18n.t
  ...
  test('Polish search resolves the exact controlled keys (łosoś, ser, paracetamol)', async () => {
    await i18n.changeLanguage('pl');
    const lososKeys = lookupSafetyLocalized(
      'łosoś',
      FOOD_SAFETY,
      'food',
      i18n.t
    ...
    const serKeys = lookupSafetyLocalized(
      'ser miękki',
      FOOD_SAFETY,
      'food',
      i18n.t
    ...
    const paracetamolKeys = lookupSafetyLocalized(
      'paracetamol',
      MED_SAFETY,
      'med',
      i18n.t
    ...
    const surowaRyba = lookupSafetyLocalized(
      'surowa ryba',
      FOOD_SAFETY,
      'food',
      i18n.t
    ...
    const nimesulidKeys = lookupSafetyLocalized(
      'nimesulid',
      MED_SAFETY,
      'med',
      i18n.t
  ...
  test('longer natural Polish queries resolve precise controlled keys', async () => {
    await i18n.changeLanguage('pl');
    const rawQuery = lookupSafetyLocalized(
      'czy mogę jeść surową rybę',
      FOOD_SAFETY,
      'food',
      i18n.t
    ).map((i) => i.key);
    ...
    const ibuQuery = lookupSafetyLocalized(
    ...
    const softCheeseQuery = lookupSafetyLocalized(
      'mam ser miękki niepasteryzowany',
      FOOD_SAFETY,
      'food',
      i18n.t
    ).map((i) => i.key);
  ...
  test('broad category words do not imply any specific controlled item', async () => {
    ...
    const ryba = lookupSafetyLocalized('ryba', FOOD_SAFETY, 'food', i18n.t).map(
      (i) => i.key
    );
    ...
    const ser = lookupSafetyLocalized('ser', FOOD_SAFETY, 'food', i18n.t).map(
      (i) => i.key
    );
    ...
    const mieso = lookupSafetyLocalized(
      'mięso',
      FOOD_SAFETY,
      'food',
      i18n.t
  ...
  test('precise Polish queries resolve their exact controlled keys (positive and negative)', async () => {
    await i18n.changeLanguage('pl');
    const twardy = lookupSafetyLocalized(
      'ser twardy',
      FOOD_SAFETY,
      'food',
      i18n.t
    ...
    const miekkie = lookupSafetyLocalized(
      'ser miękki',
      FOOD_SAFETY,
      'food',
      i18n.t
    ...
    expect(surowaRyba).toContain('sushi_raw');
    expect(surowaRyba).not.toContain('undercooked_meat');
    const niedogotowane = lookupSafetyLocalized(
      'niedogotowane mięso',
      FOOD_SAFETY,
      'food',
      i18n.t
    ...
    const losos = lookupSafetyLocalized(
      'łosoś',
      FOOD_SAFETY,
      'food',
      i18n.t
  ...
  test('brand aliases work (Tylenol, Advil, Benadryl)', async () => {
    expect(
      lookupSafetyLocalized('Tylenol', MED_SAFETY, 'med', i18n.t).length
    ).toBeGreaterThan(0);
    expect(
      lookupSafetyLocalized('Advil', MED_SAFETY, 'med', i18n.t).length
    ).toBeGreaterThan(0);
    expect(
      lookupSafetyLocalized('Benadryl', MED_SAFETY, 'med', i18n.t).length
    ).toBeGreaterThan(0);
  });
});
