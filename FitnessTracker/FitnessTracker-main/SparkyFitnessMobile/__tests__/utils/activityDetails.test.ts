...
describe('extractActivitySummary', () => {
  ...
  test('uses the supplied translator for Polish labels and interpolation', () => {
    ...
    const t = (
      key: string,
      options: { defaultValue: string; zone?: string | number }
    ) =>
      (polish[key] ?? options.defaultValue).replace(
        '{{zone}}',
        String(options.zone ?? '')
      );
    ...
    expect(
      extractActivitySummary(
        [
          activityDetail({
            detail_data: {
              activity: { averageHeartRateInBeatsPerMinute: 145 },
              hr_in_timezones: [{ zoneNumber: 3, secsInZone: 125 }],
            },
          }),
        ],
    ...
    expect(
      extractActivitySummary(
        [
          activityDetail({
            detail_data: {
              hr_in_timezones: [{ zoneNumber: 2, secsInZone: 60 }],
            },
          }),
        ],
        t as never
      )
    ).toContainEqual({ label: 'Zone 2', value: '1m 0s' });
  });
  ...
  test('extracts Withings HR zones', () => {
    const items = extractActivitySummary([
      activityDetail({
        detail_type: 'hr_zones',
        detail_data: JSON.stringify({
          hr_zones: { light: 180, moderate: 360, intense: 60, peak: 0 },
        }),
        provider_name: 'withings',
      }),
    ]);
    expect(items).toContainEqual({
      label: 'Heart-rate zone: Light',
      value: '3m 0s',
    });
    expect(items).toContainEqual({
      label: 'Heart-rate zone: Moderate',
      value: '6m 0s',
    });
  ...
  test('renders production Withings semantic zone names in Polish', () => {
    ...
    expect(
      extractActivitySummary(
        [
          activityDetail({
            detail_data: { hr_zones: { light: 60, peak: 120 } },
            provider_name: 'withings',
          }),
        ],
        t as never
  ...
  test('handles non-object parsed data as primitive', () => {
    ...
    ).toContainEqual({ label: 'step_count', value: '5000' });
  });
  ...
  test('handles Withings HR zones with non-number values', () => {
    const items = extractActivitySummary([
      activityDetail({
        detail_type: 'hr_zones',
        detail_data: JSON.stringify({
          hr_zones: { light: 'not-a-number', moderate: 300 },
        }),
        provider_name: 'withings',
      }),
    ]);
  ...
  test('localizes HR-zone durations per application locale', async () => {
    ...
    const enItems = extractActivitySummary(
      garmin(),
      makeT(enDuration) as never
    );
    // 125 seconds = 2 minutes 5 seconds.
    expect(enItems.find((i) => i.label === 'Zone 3')?.value).toBe('2m 5s');

    const plItems = extractActivitySummary(
      garmin(),
      makeT(plDuration) as never
    );
