...
describe('localizeProtocolBadge', () => {
  ...
  it('localizes known non-ratio presets', () => {
    ...
    expect(localizeProtocolBadge(pl as never, 'Custom Fast')).toBe(
      'Własny post'
    );
  });

  it('keeps arbitrary server or user-created protocol names literal', () => {
    expect(localizeProtocolBadge(pl as never, 'Mój post 14 godzin')).toBe(
      'Mój post 14 godzin'
    );
    expect(localizeProtocolBadge(pl as never, 'Custom 16:8 plan')).toBe(
      'Custom 16:8 plan'
    );
  });
  ...
  it('localizes metabolic-stage range units', () => {
    ...
    expect(localizeFastingStage(pl as never, stage).rangeLabel).toBe(
      '4–16 godz.'
    );
    ...
    expect(localizeFastingStage(pl as never, finalStage).rangeLabel).toBe(
      '72 godz.+'
    );
  });
});
...
describe('fasting protocol picker catalog copy', () => {
  ...
  test.each(names)(
    'keeps canonical EN and reviewed PL picker name for %s',
    async (key, enExpected, plExpected) => {
      await i18n.changeLanguage('en');
