...
jest.mock('../../src/components/CalendarSheet', () => {
  ...
  const MockCalendar = ReactModule.forwardRef(() => null);
  return { __esModule: true, default: MockCalendar };
});
jest.mock('../../src/components/Icon', () => () => null);
jest.mock('../../src/components/FadeView', () => ({ children }: any) => (
  <>{children}</>
));
jest.mock('../../src/components/SafeImage', () => () => null);
jest.mock('../../src/components/FormInput', () => (props: any) => {
  const { Text: RNText } = require('react-native');
  return <RNText testID="form-input">{props.placeholder}</RNText>;
});
jest.mock(
  '../../src/components/ui/Button',
  () =>
    ({ children, onPress, disabled }: any) => {
      const {
        Text: RNText,
        TouchableOpacity: RNPressable,
...
jest.mock('../../src/hooks/useScreenHeader', () => ({
  ...
  useScreenHeader: (options: any) => {
    ...
    return (
      <RNView>
        {options.right?.onPress && (
          <RNPressable onPress={options.right.onPress}>
            <RNText>
              {options.right.busy
                ? options.right.busyLabel
                : options.right.label}
            </RNText>
          </RNPressable>
        )}
        {options.left?.onPress && (
          <RNPressable onPress={options.left.onPress}>
            <RNText>{options.left.accessibilityLabel}</RNText>
          </RNPressable>
        )}
      </RNView>
    );
...
const mockDelete = useDeleteExerciseEntry as jest.MockedFunction<
  typeof useDeleteExerciseEntry
>;
const mockUpdate = useUpdateExerciseEntry as jest.MockedFunction<
  typeof useUpdateExerciseEntry
>;
...
const session = {
  ...
  source: 'manual',
  sets: [],
  exercise_snapshot: { name: 'Dragon Run', category: 'Cardio', images: [] },
  activity_details: [],
  superset_group: null,
} as any;
...
describe('ActivityDetailScreen localization', () => {
  ...
  beforeEach(async () => {
    ...
    mockDelete.mockImplementation(
      () => ({ isPending: pending, confirmAndDelete: jest.fn() }) as any
    );
