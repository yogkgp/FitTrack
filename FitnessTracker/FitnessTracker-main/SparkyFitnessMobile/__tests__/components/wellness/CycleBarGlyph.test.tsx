...
describe('CycleBarGlyph', () => {
  it('scales the track against the longest cycle in the list', () => {
    const screen = render(
      <CycleBarGlyph cycleLength={15} periodLength={5} maxLength={30} />
    );
  ...
  it('renders the longest cycle at full width', () => {
    const screen = render(
      <CycleBarGlyph cycleLength={30} periodLength={5} maxLength={30} />
    );
  ...
  it('sizes the period segment relative to its own cycle, not the max', () => {
    const screen = render(
      <CycleBarGlyph cycleLength={20} periodLength={5} maxLength={40} />
    );
  ...
  it('never exceeds full width when maxLength is smaller than the cycle', () => {
    const screen = render(
      <CycleBarGlyph cycleLength={30} periodLength={5} maxLength={0} />
    );
    expect(trackWidth(screen)).toBe('100%');
  });
});
