...
describe('foodEntryToNutrientSamples', () => {
  it('scales nutrients by quantity/serving_size and keeps native units', () => {
    ...
    expect(sampleFor(descriptor, DIETARY_ENERGY_IDENTIFIER)).toMatchObject({
      unit: 'kcal',
      quantity: 300,
    });
    expect(
      sampleFor(descriptor, 'HKQuantityTypeIdentifierDietaryProtein')
    ).toMatchObject({ unit: 'g', quantity: 15 });
    // mg/mcg columns are written in their native unit with the value unchanged.
    expect(
      sampleFor(descriptor, 'HKQuantityTypeIdentifierDietarySodium')
    ).toMatchObject({ unit: 'mg', quantity: 600 });
    expect(
      sampleFor(descriptor, 'HKQuantityTypeIdentifierDietaryVitaminA')
    ).toMatchObject({ unit: 'mcg', quantity: 120 });
  });

  it('omits zero / undefined nutrients', () => {
    const descriptor = foodEntryToNutrientSamples(baseEntry, NOW)!;
    expect(
      sampleFor(descriptor, 'HKQuantityTypeIdentifierDietaryFatMonounsaturated')
    ).toBeUndefined(); // 0
    expect(
      sampleFor(descriptor, 'HKQuantityTypeIdentifierDietaryCholesterol')
  ...
  it('never emits a trans-fat sample (no HealthKit identifier in the library)', () => {
    ...
    expect(
      descriptor.samples.every((s) => !/Trans/i.test(s.quantityType))
    ).toBe(true);
  });

  it('returns null when serving_size is 0 (cannot scale)', () => {
    expect(
      foodEntryToNutrientSamples({ ...baseEntry, serving_size: 0 }, NOW)
    ).toBeNull();
  });
  ...
  it('defers (returns null) when the meal-time anchor is still in the future', () => {
    ...
    expect(
      foodEntryToNutrientSamples(
        { ...baseEntry, meal_type: 'dinner', entry_date: '2026-06-15' },
        now
      )
    ).toBeNull();
  });
  ...
  it('anchors each meal to its representative local time', () => {
    ...
    const dinner = foodEntryToNutrientSamples(
      { ...baseEntry, meal_type: 'dinner' },
      NOW
    )!;
    expect(dinner.start.getHours()).toBe(19);
    // Unknown meal types fall back to the snack anchor (15:00).
    const custom = foodEntryToNutrientSamples(
      { ...baseEntry, meal_type: 'pre-workout' },
      NOW
    )!;
...
describe('DIETARY_HK_MAP', () => {
  ...
  it('DIETARY_WRITE_IDENTIFIERS leads with energy and contains no trans-fat id', () => {
    ...
    expect(DIETARY_WRITE_IDENTIFIERS.some((id) => /Trans/i.test(id))).toBe(
      false
    );
  });
});
