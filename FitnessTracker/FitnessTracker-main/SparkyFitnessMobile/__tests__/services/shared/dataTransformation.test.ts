...
const baseConfig = (
  overrides: Partial<TransformHealthRecordsConfig> = {}
): TransformHealthRecordsConfig => ({
  source: 'Test Source',
  logTag: '[TestService]',
  valueTransformers: {},
  directTransformers: {},
...
describe('createTransformHealthRecords', () => {
  describe('skipTypes', () => {
    ...
    test('drops configured skip types wholesale and logs', () => {
      const transform = createTransformHealthRecords(
        baseConfig({ skipTypes: new Set(['Widget']) })
      );
      expect(transform(records, METRIC)).toEqual([]);
      expect(mockAddLog).toHaveBeenCalledWith(
        '[TestService] Skipping qualitative Widget records'
      );
    });
    ...
    test('non-listed record types are unaffected by skipTypes', () => {
      const transform = createTransformHealthRecords(
        baseConfig({ skipTypes: new Set(['Other']) })
      );
      expect(transform(records, METRIC)).toHaveLength(1);
    });
  });
  ...
  describe('source stamping', () => {
    test('stamps config source on pre-aggregated records', () => {
      const transform = createTransformHealthRecords(
        baseConfig({ source: 'Health Connect' })
      );
    ...
    test('stamps config source on value-transformed records', () => {
      const transform = createTransformHealthRecords(
        baseConfig({
          source: 'HealthKit',
          valueTransformers: {
            Widget: () => ({ value: 7, date: '2024-01-15' }),
          },
        })
      );
  ...
  describe('injected timezone extractor', () => {
      ...
      const transform = createTransformHealthRecords(
        baseConfig({
          ...
          extractTimezoneMetadata: (rec) => ({
            record_timezone: rec.tz as string,
          }),
        })
      );
    ...
    test('pre-aggregated records forward their own tz fields, not the extractor', () => {
      const extractTimezoneMetadata = jest.fn(() => ({
        record_timezone: 'Wrong/Zone',
      }));
      const transform = createTransformHealthRecords(
        baseConfig({ extractTimezoneMetadata })
      );
  ...
  describe('index forwarding', () => {
    test('value transformers receive the record index as third argument', () => {
      const valueTransformer: jest.MockedFunction<ValueTransformer> = jest.fn(
  ...
  describe('direct-transformer counting', () => {
    test('counts pushed records as successes in the summary log', () => {
      ...
      const result = transform(
        [{ raw: true }, { value: NaN, date: '2024-01-15' }],
        METRIC
      );
  ...
  describe('driver guards', () => {
    ...
    test('warns once for unhandled record types and skips them', () => {
      ...
      const unhandledWarnings = mockAddLog.mock.calls.filter(
        ([message]) =>
          message ===
          '[TestService] No transformer found for record type: Widget'
      );
...
describe('createHydrationTransformer', () => {
  ...
  test('converts liters to whole ml', () => {
    ...
    expect(
      transformer({ volume: { inLiters: 0.7501 }, startTime: 'x' }, METRIC, 0)
  ...
  test('reads source_id from a top-level uuid (iOS HealthKit)', () => {
    ...
    expect(
      transformer(
        { volume: { inLiters: 1 }, startTime: 'x', uuid: 'hk-uuid-1' },
        METRIC,
        0
      )
    ).toEqual({
  ...
  test('omits source_id when no id is available anywhere', () => {
    const transformer = createHydrationTransformer(() => false, getDateString);
    expect(
      transformer({ volume: { inLiters: 1 }, startTime: 'x' }, METRIC, 0)
    ).toEqual({ value: 1000, date: '2024-01-15', timestamp: 'x' });
  });

  test('skips records matched by the injected ownership predicate', () => {
    const transformer = createHydrationTransformer(
      (rec) => rec.mine === true,
      getDateString
    );
    expect(
      transformer(
        { mine: true, volume: { inLiters: 1 }, startTime: 'x' },
        METRIC,
        0
      )
    ).toBeNull();
    expect(
      transformer(
        { mine: false, volume: { inLiters: 1 }, startTime: 'x' },
        METRIC,
        0
      )
    ).toEqual({ value: 1000, date: '2024-01-15', timestamp: 'x' });
...
describe('createBloodPressureTransformer', () => {
  test('emits systolic/diastolic records stamped with the injected source', () => {
    const transformer = createBloodPressureTransformer(
      'HealthKit',
      () => '2024-01-15'
    );
    ...
    transformer(
      rec,
      rec,
      { recordType: 'BloodPressure', unit: 'mmHg', type: 'blood_pressure' },
      output
    );
