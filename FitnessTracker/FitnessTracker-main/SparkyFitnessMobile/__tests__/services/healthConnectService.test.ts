...
const mockEmptyReadsWithProbe = () => {
  mockReadRecords.mockImplementation(
    (_recordType: string, options?: { pageSize?: number }) =>
      Promise.resolve(
        options?.pageSize === 1
          ? { records: [offsetlessProbeRecord] }
          : { records: [] }
      )
  );
};
...
describe('healthConnectService.ts (Android)', () => {
  ...
  describe('syncHealthData', () => {
    ...
    test('HeartRateVariabilityRmssd records are aggregated with min/max/avg by date', async () => {
      ...
      const payload = mockApiSyncHealthData.mock.calls[0][0];
      const hrvMin = payload.find(
        (r: { type: string }) => r.type === 'HRV_min'
      );
      const hrvMax = payload.find(
        (r: { type: string }) => r.type === 'HRV_max'
      );
      const hrvAvg = payload.find(
        (r: { type: string }) => r.type === 'HRV_avg'
      );
        ...
        records: [
          {
            ...
            basalMetabolicRate: { inKilocaloriesPerDay: 1650 },
          },
        ],
      });

      await androidService.syncHealthData('today', {
        isBasalMetabolicRateSyncEnabled: true,
      });
    ...
    describe('nutrition rolling lookback', () => {
      ...
      test('Nutrition reads widen to the 2-day day-aligned lookback while other raw reads keep the session window', async () => {
        ...
        expect(callFor('Nutrition')?.[1].timeRangeFilter.startTime).toBe(
        ...
        expect(callFor('HeartRate')?.[1].timeRangeFilter.startTime).toBe(
          new Date(2026, 6, 3, 0, 0, 0, 0).toISOString()
        );
      });
    });
                  ...
                  records: [
                    {
                      startTime: probeInstant.toISOString(),
                      startZoneOffset: {
                        totalSeconds: deviceOffsetMinutes * 60,
                      },
                    },
                  ],
                }
