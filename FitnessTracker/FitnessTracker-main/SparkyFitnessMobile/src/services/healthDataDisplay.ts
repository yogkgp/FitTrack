...
const t = (key: string, options: Record<string, unknown> = {}): string => {
  ...
  const templates: Record<string, string> = {
    'healthDataDisplay.distance': '{{value}} km',
    'healthDataDisplay.kcal': '{{value}} kcal',
    'healthDataDisplay.bpm': '{{value}} bpm',
    'healthDataDisplay.kg': '{{value}} kg',
    'healthDataDisplay.percent': '{{value}}%',
    'healthDataDisplay.celsius': '{{value}}°C',
    'healthDataDisplay.mmHg': '{{systolic}}/{{diastolic}} mmHg',
    'healthDataDisplay.liters': '{{value}} L',
    'healthDataDisplay.cm': '{{value}} cm',
    'healthDataDisplay.mmolL': '{{value}} mmol/L',
    'healthDataDisplay.ms': '{{value}} ms',
    'healthDataDisplay.vo2': '{{value}} ml/min/kg',
    'healthDataDisplay.minutes': '{{value}} min',
    'healthDataDisplay.meters': '{{value}} m',
    'healthDataDisplay.watts': '{{value}} W',
    'healthDataDisplay.metersPerSecond': '{{value}} m/s',
    'healthDataDisplay.breathsPerMinute': '{{value}}/min',
    'healthDataDisplay.error': 'Error',
    'healthDataDisplay.sleepHours': '{{count}}h',
    'healthDataDisplay.sleepMinutes': '{{count}}m',
    'healthDataDisplay.workouts': '{{count}} workouts',
    'healthDataDisplay.records': '{{count}} records',
  };
...
const unit = (
  ...
  count: number,
  defaultValue: string,
  defaultValuePlural?: string
): string => {
  const fallback =
    count === 1 || !defaultValuePlural ? defaultValue : defaultValuePlural;
  ...
  const value = i18n.t(key, {
    ...
    ...(defaultValuePlural
      ? {
          defaultValue_one: defaultValue,
          defaultValue_other: defaultValuePlural,
        }
      : {}),
  });
...
const AGGREGATED_FORMATTERS: Record<
  ...
  (start: Date, end: Date) => Promise<string>
> = {
  Steps: makeAggregatedFormatter(getAggregatedStepsByDate, (value) =>
    number(value)
  ),
  ...
  Distance: makeAggregatedFormatter(getAggregatedDistanceByDate, (value) =>
    t('healthDataDisplay.distance', {
  ...
  BasalMetabolicRate: async (start, end) => {
    ...
    if (aggregated.length > 0) {
      const avg =
        aggregated.reduce((sum: number, r) => sum + r.value, 0) /
        aggregated.length;
      return t('healthDataDisplay.kcal', {
        defaultValue: '{{value}} kcal',
        value: number(Math.round(avg)),
      });
    }
    const rawRecords = (await readHealthRecords(
      'BasalMetabolicRate',
      start,
      end
    )) as unknown[];
...
const RAW_FORMATTERS: Record<string, (records: unknown[]) => string> = {
  ...
  BloodPressure: (records) => {
    ...
    return systolic && diastolic
      ? t('healthDataDisplay.mmHg', {
          defaultValue: '{{systolic}}/{{diastolic}} mmHg',
          systolic: number(Math.round(systolic)),
          diastolic: number(Math.round(diastolic)),
        })
      : NO_DATA_DISPLAY;
  },
  ...
  Hydration: (records) => {
    ...
    return totalHydration === 0
      ? NO_DATA_DISPLAY
      : t('healthDataDisplay.liters', {
          defaultValue: '{{value}} L',
          value: number(totalHydration, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }),
        });
  ...
  RestingHeartRate: (records) => {
    ...
    return avgRestingHR > 0
      ? t('healthDataDisplay.bpm', {
          defaultValue: '{{value}} bpm',
          value: number(Math.round(avgRestingHR)),
        })
      : NO_DATA_DISPLAY;
  },
  ...
  HeartRateVariabilityRmssd: (records) => {
    const latest = (
      records as { time?: string; heartRateVariabilityMillis?: number }[]
    )
      .filter(
        (r) =>
          r.heartRateVariabilityMillis != null &&
          !isNaN(r.heartRateVariabilityMillis)
      )
      .sort(
        (a, b) =>
          new Date(b.time ?? 0).getTime() - new Date(a.time ?? 0).getTime()
  ...
  Vo2Max: (records) => {
    ...
    return valid.length > 0
      ? t('healthDataDisplay.vo2', {
          defaultValue: '{{value}} ml/min/kg',
          value: number(valid[0].value!, {
            minimumFractionDigits: 1,
            maximumFractionDigits: 1,
          }),
  ...
  ElevationGained: (records) => {
    ...
    return totalElevation === 0
      ? NO_DATA_DISPLAY
      : t('healthDataDisplay.meters', {
          defaultValue: '{{value}} m',
          value: number(Math.round(totalElevation)),
        });
  },
  ...
  Power: (records) => {
    ...
    return avgPower === 0
      ? NO_DATA_DISPLAY
      : t('healthDataDisplay.watts', {
          defaultValue: '{{value}} W',
          value: number(Math.round(avgPower)),
        });
  },
  ...
  Speed: (records) => {
    ...
    return avgSpeed === 0
      ? NO_DATA_DISPLAY
      : t('healthDataDisplay.metersPerSecond', {
          defaultValue: '{{value}} m/s',
          value: number(avgSpeed, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }),
        });
  ...
  RespiratoryRate: (records) => {
    ...
    return avgRespRate === 0
      ? NO_DATA_DISPLAY
      : t('healthDataDisplay.breathsPerMinute', {
          defaultValue: '{{value}}/min',
          value: number(Math.round(avgRespRate)),
        });
  },
  ...
  Nutrition: (records) => {
    ...
    return totalNutrition === 0
      ? NO_DATA_DISPLAY
      : t('healthDataDisplay.kcal', {
          defaultValue: '{{value}} kcal',
          value: number(Math.round(totalNutrition / 1000)),
        });
  },

  Workout: (records) =>
    unit(
      'healthDataDisplay.workouts',
      records.length,
      '{{count}} workout',
      '{{count}} workouts'
    ),
...
export async function fetchHealthDisplayData(
  ...
  for (const metric of HEALTH_METRICS) {
    try {
      ...
      addLog(
        `[healthDataDisplay] Error fetching ${metric.defaultLabel}: ${errorMessage}`,
        'ERROR'
      );
      result[metric.id] = i18n.t('healthDataDisplay.error', {
        defaultValue: 'Error',
      });
    }
  }
