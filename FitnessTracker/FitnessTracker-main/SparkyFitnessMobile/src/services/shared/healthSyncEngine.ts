...
const collectMetric = async (
  ...
  if (readKind === 'cumulative-day') {
    const result = await provider.readCumulativeByDay(
      metric,
  ...
  if (readKind === 'min-max-avg-day') {
    const statsResult = await provider.readMinMaxAvgByDay(
      metric,
      windows.aggregatedStart,
      windows.end
    );
    ...
    ? new Date(
        Math.min(
          ...
          alignToLocalDayStart(
            new Date(
              windows.end.getTime() -
                metric.rollingLookbackDays * 24 * 60 * 60 * 1000
            )
          ).getTime()
        )
      )
  ...
  const processed = await provider.postProcessRaw(
    metric,
    rawRecords,
    telemetry
  );
...
export const sessionTelemetryOutcomesUsable = (
  ...
  outcomes
    .filter((outcome) =>
      TELEMETRY_STAGING_RECORD_TYPES.has(outcome.metric.recordType)
    )
...
export const runForegroundSync = async (
  ...
  const enabledMetricStates =
    healthMetricStates && typeof healthMetricStates === 'object'
      ? healthMetricStates
      : {};
  const metricsToSync = HEALTH_METRICS.filter(
  ...
  for (const outcome of outcomes) {
    ...
    if (outcome.status === 'rejected') {
      addLog(
        `${opts.logTag} Error processing ${type}: ${outcome.error}`,
        'ERROR'
      );
  ...
  if (allTransformedData.length > 0) {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      addLog(
        `${opts.logTag} Error sending data to server: ${message}`,
        'ERROR'
      );
