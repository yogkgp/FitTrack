...
const WorkoutAddScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (!isEditMode || !session || hasPopulatedEdit || isPreferencesLoading) {
      return;
    }
  ...
  const handleFinish = useCallback(() => {
    if (!submission.canSave) {
      Toast.show({
        type: 'error',
        text1: t('workoutAdd.addExercise', { defaultValue: 'Add an Exercise' }),
        text2: t('workoutAdd.addExerciseMessage', {
    ...
    const alertTitle = isEditMode
      ? t('workoutAdd.saveChangesTitle', { defaultValue: 'Save Changes?' })
      : t('workoutAdd.saveWorkoutTitle', { defaultValue: 'Save Workout?' });
    const alertMessage = `Save "${submission.name}" with ${submission.exerciseCount} exercise(s)?`;

    Alert.alert(alertTitle, alertMessage, [
      {
        text: t('workoutAdd.cancel', { defaultValue: 'Cancel' }),
        style: 'cancel',
      },
      {
        ...
        onPress: async () => {
          try {
            ...
            Toast.show({
              type: 'error',
              text1: t('workoutAdd.saveFailed', {
                defaultValue: 'Failed to save workout',
              }),
              text2: t('workoutAdd.tryAgain', {
      ...
      ? [
          {
            ...
            accessibilityLabel: t('workoutAdd.reorderExercises', {
            ...
            >
                ...
                {isNameEditing ? (
                  <FadeView key="name-edit">
                    <FormInput
                      ...
                      placeholder={t('workoutAdd.workoutPlaceholder', {
                  ...
                  <FadeView key="name-view">
                    ...
                    >
                      <Text className="text-xl font-bold text-text-primary">
                        {state.name ||
                          t('workoutAdd.workoutPlaceholder', {
                            defaultValue: 'Workout',
                          })}
                      </Text>
              ...
              <View className="flex-row items-center mb-4">
                ...
                {state.entryDate === getTodayDate() ? (
                  ...
                  >
                    <Text className="text-text-link text-sm font-medium mx-1.5">
                      {t('workoutAdd.useToday', { defaultValue: 'Use Today' })}
                    </Text>
                  </TouchableOpacity>
                )}
              </View>
