...
import WeekdaySheet, {
  type WeekdaySheetRef,
} from '../components/medications/WeekdaySheet';
...
type MedicationScheduleFormScreenProps =
  RootStackScreenProps<'MedicationScheduleForm'>;
...
const usesDaysOfWeek = (type: string) =>
  type === 'weekly' || type === 'specific_days';
...
const requiresStartDate = (type: string) =>
  type === 'every_n_days' || type === 'cyclic';
...
function baseFromSchedule(existing?: MedicationSchedule): FormState {
  ...
  return {
    ...
    doseAmount:
      existing.dose_amount != null ? String(existing.dose_amount) : '',
    daysOfWeek: existing.days_of_week ?? [],
    intervalDays:
      existing.interval_days != null
        ? String(existing.interval_days)
        : EMPTY_FORM.intervalDays,
    dayOfMonth:
      existing.day_of_month != null
        ? String(existing.day_of_month)
        : EMPTY_FORM.dayOfMonth,
    cycleOnDays:
      existing.cycle_on_days != null
        ? String(existing.cycle_on_days)
        : EMPTY_FORM.cycleOnDays,
    cycleOffDays:
      existing.cycle_off_days != null
        ? String(existing.cycle_off_days)
        : EMPTY_FORM.cycleOffDays,
    withMeal: existing.with_meal ?? '',
    prnReason: existing.prn_reason ?? '',
    prnMaxPerDay:
      existing.prn_max_per_day != null ? String(existing.prn_max_per_day) : '',
...
> = ({ route, navigation }) => {
  ...
  const buildPayload = useCallback((): CreateScheduleInput => {
    ...
    return {
      ...
      interval_days: usesIntervalDays(type)
        ? parseInt(form.intervalDays, 10)
        : null,
      ...
      prn_max_per_day:
        isPrnType(type) && form.prnMaxPerDay
          ? parseInt(form.prnMaxPerDay, 10)
          : null,
    };
  }, [form, existing]);

  const handleSave = useCallback(() => {
    if (createScheduleMutation.isPending || updateScheduleMutation.isPending)
      return;
    ...
    if (usesDaysOfWeek(type) && form.daysOfWeek.length === 0) {
      Alert.alert(
        t('medications.schedule.required', { defaultValue: 'Required' }),
        t('medications.schedule.daysRequired', {
          defaultValue: 'Select at least one day of the week.',
        })
      );
      return;
    }
    if (usesIntervalDays(type)) {
      ...
      if (!Number.isFinite(interval) || interval < 1) {
        Alert.alert(
          t('medications.schedule.invalidInterval', {
            defaultValue: 'Invalid interval',
          }),
          t('medications.schedule.intervalMessage', {
            defaultValue: 'Interval must be at least 1 day.',
          })
        );
        return;
      }
      if (!form.startDate) {
        Alert.alert(
          t('medications.schedule.required', { defaultValue: 'Required' }),
          t('medications.schedule.everyNStart', {
            defaultValue: 'Every N days schedules need a start date.',
          })
        );
        return;
      }
    ...
    if (usesCycle(type)) {
      ...
      if (!Number.isFinite(on) || on < 1 || !Number.isFinite(off) || off < 0) {
        Alert.alert(
          t('medications.schedule.invalidCycle', {
            defaultValue: 'Invalid cycle',
          }),
          t('medications.schedule.cycleMessage', {
            defaultValue: 'Days on must be at least 1 and days off at least 0.',
          })
        );
        return;
      }
      if (!form.startDate) {
        Alert.alert(
          t('medications.schedule.required', { defaultValue: 'Required' }),
          t('medications.schedule.cycleStart', {
            defaultValue: 'Cycle schedules need a start date.',
          })
        );
        return;
      }
    ...
    if (usesDayOfMonth(type)) {
      ...
      if (!Number.isFinite(day) || day < 1 || day > 31) {
        Alert.alert(
          t('medications.schedule.invalidDay', { defaultValue: 'Invalid day' }),
          t('medications.schedule.dayMessage', {
            defaultValue: 'Day of month must be between 1 and 31.',
          })
        );
        return;
      }
    ...
    if (form.doseAmount) {
      ...
      if (!Number.isFinite(dose) || dose <= 0) {
        Alert.alert(
          t('medications.schedule.invalidDose', {
            defaultValue: 'Invalid dose',
          }),
          t('medications.schedule.doseMessage', {
            defaultValue: 'Please enter a valid dose amount.',
          })
        );
        return;
      }
    ...
    if (isPrnType(type) && form.prnMaxPerDay) {
      ...
      if (!Number.isFinite(max) || max < 1) {
        Alert.alert(
          t('medications.schedule.invalidLimit', {
            defaultValue: 'Invalid limit',
          }),
          t('medications.schedule.limitMessage', {
            defaultValue: 'Max per day must be at least 1.',
          })
        );
        return;
      }
    ...
    if (form.startDate && form.endDate && form.endDate < form.startDate) {
      Alert.alert(
        t('medications.schedule.invalidDates', {
          defaultValue: 'Invalid dates',
        }),
        t('medications.schedule.datesMessage', {
        ...
        { id: scheduleId, medicationId, body },
        {
          ...
          onError: (error) =>
            Alert.alert(
              ...
              t('medications.schedule.updateFailed', {
                defaultValue: 'Failed to update schedule: {{error}}',
                error: error.message,
              })
            ),
        }
      ...
      createScheduleMutation.mutate(
        { medicationId, body },
        {
          ...
          onError: (error) =>
            Alert.alert(
              t('common.error', { defaultValue: 'Error' }),
              t('medications.schedule.createFailed', {
                defaultValue: 'Failed to create schedule: {{error}}',
                error: error.message,
              })
            ),
        }
  ...
  const handleDelete = useCallback(() => {
    ...
    Alert.alert(
      t('medications.schedule.deleteTitle', {
        defaultValue: 'Delete Schedule',
      }),
      t('medications.schedule.deleteMessage', {
        defaultValue: 'Delete this schedule? Logged doses are kept.',
      }),
      [
        {
            ...
            deleteScheduleMutation.mutate(
              { id: scheduleId, medicationId },
              {
                ...
                onError: (error) => {
                  ...
                  Toast.show({
                    type: 'error',
                    text1: t('medications.schedule.deleteFailed', {
  ...
  const typeOptions = useMemo(() => {
    const options: { label: string; value: string }[] = SCHEDULE_TYPES.map(
      (id) => ({
    ...
    if (
      form.scheduleTypeId &&
      !options.some((o) => o.value === form.scheduleTypeId)
    ) {
      options.push({
        label: humanizeTypeId(form.scheduleTypeId),
  ...
  const withMealOptions = useMemo(
    () => [
      {
        label: t('medications.schedule.none', { defaultValue: 'None' }),
        value: '',
      },
      ...
      {
        label: localizedMealTimingLabel(t, 'away_from_meals'),
        value: 'away_from_meals',
      },
    ],
    [t]
  );
      ...
      >
        ...
        <View className="flex-1 items-center justify-center">
          <Text className="text-text-muted text-base">
            {isLoading || !med
              ? t('medications.schedule.loading', {
                  defaultValue: 'Loading...',
                })
              : t('medications.schedule.notFound', {
      ...
      >
        <SettingsRowGroup>
          {!isPrnType(type) && (
            <SettingsRow
              ...
              accessibilityLabel={t('medications.schedule.setTime', {
                ...
                <View className="flex-row items-center gap-3">
                  {form.timeOfDay !== '' && (
                    <TouchableOpacity
                      ...
                      accessibilityLabel={t('medications.schedule.clearTime', {
                        defaultValue: 'Clear time',
                      })}
                    >
                      <Text className="text-text-link text-sm font-medium">
                        {t('medications.schedule.clear', {
                          defaultValue: 'Clear',
                        })}
                      </Text>
                    </TouchableOpacity>
          ...
          <BottomSheetPicker
            ...
            title={t('medications.schedule.scheduleType', {
              defaultValue: 'Schedule Type',
            })}
            renderTrigger={({ onPress, selectedOption }) => (
              <SettingsRow
                title={t('medications.schedule.frequency', {
          ...
          {usesDaysOfWeek(type) && (
            <SettingsRow
              title={t('medications.schedule.daysOfWeek', {
                defaultValue: 'Days of week',
              })}
              accessibilityLabel={t('medications.schedule.selectDays', {
                defaultValue: 'Select days of week',
              })}
          ...
          {usesIntervalDays(type) && (
            <SettingsRow
              title={t('medications.schedule.everyNDays', {
              ...
              rightAccessory={
                <StepperInput
                  ...
                  onIncrement={() =>
                    stepInt('intervalDays', form.intervalDays, 1, 1)
                  }
                  onDecrement={() =>
                    stepInt('intervalDays', form.intervalDays, -1, 1)
                  }
          ...
          {usesDayOfMonth(type) && (
            <SettingsRow
              title={t('medications.schedule.dayOfMonth', {
              ...
              rightAccessory={
                <StepperInput
                  ...
                  onIncrement={() =>
                    stepInt('dayOfMonth', form.dayOfMonth, 1, 1, 31)
                  }
                  onDecrement={() =>
                    stepInt('dayOfMonth', form.dayOfMonth, -1, 1, 31)
                  }
          ...
          {usesCycle(type) && (
            <SettingsRow
              title={t('medications.schedule.daysOn', {
                defaultValue: 'Days on',
              })}
              rightAccessory={
                <StepperInput
                  ...
                  onIncrement={() =>
                    stepInt('cycleOnDays', form.cycleOnDays, 1, 1)
                  }
                  onDecrement={() =>
                    stepInt('cycleOnDays', form.cycleOnDays, -1, 1)
                  }
          ...
          {usesCycle(type) && (
            <SettingsRow
              title={t('medications.schedule.daysOff', {
              ...
              rightAccessory={
                <StepperInput
                  ...
                  onIncrement={() =>
                    stepInt('cycleOffDays', form.cycleOffDays, 1, 0)
                  }
                  onDecrement={() =>
                    stepInt('cycleOffDays', form.cycleOffDays, -1, 0)
                  }
          ...
          {isPrnType(type) && (
            <SettingsRow
              title={t('medications.schedule.reason', {
                defaultValue: 'Reason',
              })}
              rightAccessory={
                <FormInput
                  placeholder={t('medications.schedule.reasonPlaceholder', {
          ...
          {isPrnType(type) && (
            <SettingsRow
              title={t('medications.schedule.maxPerDay', {
                defaultValue: 'Max per day',
              })}
              rightAccessory={
                <StepperInput
                  ...
                  onIncrement={() =>
                    stepInt('prnMaxPerDay', form.prnMaxPerDay, 1, 1)
                  }
                  onDecrement={() =>
                    stepInt('prnMaxPerDay', form.prnMaxPerDay, -1, 1)
                  }
                  keyboardType="number-pad"
                  placeholder={t('medications.schedule.noLimit', {
        ...
        {!isPrnType(type) && form.timeOfDay === '' && (
          <Text className="text-sm text-text-muted px-4 -mt-2 mb-4">
            {t('medications.schedule.noTimeReminder', {
              defaultValue: "A schedule without a time won't get a reminder.",
            })}
          </Text>
        )}
        ...
        <SettingsRowGroup>
          <SettingsRow
            title={
              med?.dose_unit
                ? t('medications.schedule.doseOverride', {
                    defaultValue: 'Dose override ({{unit}})',
                    unit: med.dose_unit,
                  })
                : t('medications.schedule.doseOverridePlain', {
                    defaultValue: 'Dose override',
                  })
            }
            rightAccessory={
              <FormInput
                placeholder={t('medications.schedule.default', {
          ...
          <BottomSheetPicker
            ...
            title={t('medications.schedule.withMealTitle', {
              defaultValue: 'With Meal',
            })}
            renderTrigger={({ onPress, selectedOption }) => (
              <SettingsRow
                title={t('medications.schedule.withMeal', {
                  defaultValue: 'With meal',
                })}
                onPress={onPress}
                rightAccessory={
        ...
        <SettingsRowGroup>
          <SettingsRow
            title={
              startRequired
                ? t('medications.schedule.startDateRequired', {
                    defaultValue: 'Start date *',
                  })
                : t('medications.schedule.startDate', {
                    defaultValue: 'Start date',
                  })
            }
            accessibilityLabel={t('medications.schedule.setStartDate', {
            ...
            rightAccessory={
              <View className="flex-row items-center gap-3">
                {form.startDate !== '' && !startRequired && (
                  <TouchableOpacity
                    ...
                    accessibilityLabel={t(
                      'medications.schedule.clearStartDate',
                      { defaultValue: 'Clear start date' }
                    )}
                  >
                    <Text className="text-text-link text-sm font-medium">
                      {t('medications.schedule.clear', {
                ...
                <Text className="text-base text-text-secondary">
                  {form.startDate
                    ? formatDateLabel(form.startDate, t, dateLocale)
                    : t('medications.schedule.none', { defaultValue: 'None' })}
                </Text>
              </View>
            }
          />
          <SettingsRow
            title={t('medications.schedule.endDate', {
              defaultValue: 'End date',
            })}
            accessibilityLabel={t('medications.schedule.setEndDate', {
              ...
              <View className="flex-row items-center gap-3">
                {form.endDate !== '' && (
                  <TouchableOpacity
                    ...
                    accessibilityRole="button"
                    accessibilityLabel={t('medications.schedule.clearEndDate', {
                ...
                <Text className="text-base text-text-secondary">
                  {form.endDate
                    ? formatDateLabel(form.endDate, t, dateLocale)
                    : t('medications.schedule.none', { defaultValue: 'None' })}
                </Text>
              </View>
            }
        ...
        {isEditing && (
          <SettingsRowGroup>
            <SettingsRow
              title={t('medications.schedule.active', {
                defaultValue: 'Active',
              })}
              rightAccessory={
                <Switch
