...
} from '../../../hooks/usePregnancyChecklist';
...
function localizedChecklistTitle(
  ...
  switch (key) {
    case 'first_appt':
      return t('pregnancy.checklist.items.firstAppt', {
        defaultValue: 'Book your first prenatal appointment',
      });
    case 'prenatal_vitamin':
      return t('pregnancy.checklist.items.prenatalVitamin', {
        defaultValue: 'Start a prenatal vitamin with folic acid',
      });
    case 'nt_scan':
      return t('pregnancy.checklist.items.ntScan', {
        defaultValue: 'Schedule first-trimester screening',
      });
    case 'share_news':
      return t('pregnancy.checklist.items.shareNews', {
        defaultValue: 'Share your news if you’re ready',
      });
    case 'anatomy_scan':
      return t('pregnancy.checklist.items.anatomyScan', {
        defaultValue: 'Attend your anatomy-scan ultrasound',
      });
    case 'glucose_test':
      return t('pregnancy.checklist.items.glucoseTest', {
        defaultValue: 'Book your glucose screening test',
      });
    case 'count_kicks':
      return t('pregnancy.checklist.items.countKicks', {
        defaultValue: 'Start counting fetal kicks daily',
      });
    case 'birth_class':
      return t('pregnancy.checklist.items.birthClass', {
        defaultValue: 'Enroll in a birth or parenting class',
      });
    case 'birth_plan':
      return t('pregnancy.checklist.items.birthPlan', {
        defaultValue: 'Draft your birth plan',
      });
    case 'hospital_bag':
      return t('pregnancy.checklist.items.hospitalBag', {
        defaultValue: 'Pack your hospital bag',
      });
    case 'install_car_seat':
      return t('pregnancy.checklist.items.installCarSeat', {
        defaultValue: 'Install and check the car seat',
      });
    case 'pediatrician':
      return t('pregnancy.checklist.items.pediatrician', {
        defaultValue: 'Choose a pediatrician',
      });
    default:
      return fallback;
  }
...
const WeeklyChecklist: React.FC<WeeklyChecklistProps> = ({
  ...
  const rows = useMemo<ChecklistRow[]>(() => {
    ...
    const byKey = new Map(
      items
        .filter((i) => i.template_key)
        .map((i) => [i.template_key as string, i])
    );
    ...
    const pastCompleted: ChecklistRow[] = items
      .filter(
        (i) =>
          i.template_key && i.completed_at && !windowKeys.has(i.template_key)
      )
      .map((i) => {
    ...
    <View className="bg-surface rounded-xl p-4 shadow-sm">
      ...
      {isLoading ? (
        ...
        <Text className="text-text-secondary text-sm py-2">
          {t('pregnancy.checklist.empty', {
            defaultValue: 'Nothing on your checklist for this week.',
          })}
        </Text>
      ) : (
        rows.map((row) => (
          ...
          >
            <View
              ...
              style={{
                borderWidth: 1.5,
                borderColor: row.completed ? iconSuccess : iconDecorative,
              }}
            >
              {row.completed && (
                <Icon
