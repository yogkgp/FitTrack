...
export interface PregnancyDueDateFormState {
  ...
  dates: Pick<
    SharedPregnancy,
    'due_date' | 'due_date_basis' | 'lmp_date' | 'conception_date'
  >;
}
...
export function usePregnancyDueDateForm(
  ...
  const validate = (): string | null => {
    ...
    if (basis === 'lmp' && compareDays(date, today) > 0) {
      return t('pregnancyDueDate.errors.lmpFuture', {
        defaultValue: 'Your last period can’t be in the future.',
      });
    }
    if (basis === 'conception' && compareDays(date, today) > 0) {
      return t('pregnancyDueDate.errors.conceptionFuture', {
        defaultValue: 'The conception date can’t be in the future.',
      });
    }
    if (compareDays(computedDueDate, addDays(today, -MAX_OVERDUE_DAYS)) < 0) {
      return t('pregnancyDueDate.errors.past', {
        defaultValue: 'That due date is in the past. Please check the date.',
      });
    }
    if (compareDays(computedDueDate, addDays(today, MAX_DUE_DAYS_AHEAD)) > 0) {
      return t('pregnancyDueDate.errors.tooFar', {
        defaultValue:
          'That due date is too far away — a pregnancy is about 40 weeks.',
      });
    }
    return null;
  };
 ...
 */
const PregnancyDueDateForm: React.FC<PregnancyDueDateFormProps> = ({
  form,
  children,
}) => {
      ...
      <SettingsRowGroup>
          ...
          rightAccessory={
            <BottomSheetPicker
              ...
              options={BASIS_OPTIONS.map((option) => ({
                ...
                label: (
                  {
                    dueDate: t('pregnancyDueDate.options.dueDate', {
                      defaultValue: 'Due date',
                    }),
                    ultrasoundScan: t(
                      'pregnancyDueDate.options.ultrasoundScan',
                      { defaultValue: 'Ultrasound scan' }
                    ),
                    lastPeriod: t('pregnancyDueDate.options.lastPeriod', {
                      defaultValue: 'Last period (LMP)',
                    }),
                    conceptionDate: t(
                      'pregnancyDueDate.options.conceptionDate',
                      { defaultValue: 'Conception date' }
                    ),
                  } as Record<string, string>
                )[option.label],
              }))}
              onSelect={form.setBasis}
              title={t('pregnancyDueDate.estimateBy', {
                defaultValue: 'Estimate due date by',
              })}
        ...
        <SettingsRow
          title={
            (
              {
                firstDayLastPeriod: t(
                  'pregnancyDueDate.fields.firstDayLastPeriod',
                  { defaultValue: 'First day of last period' }
                ),
                conceptionDate: t('pregnancyDueDate.fields.conceptionDate', {
                  defaultValue: 'Conception date',
                }),
                estimatedDueDate: t(
                  'pregnancyDueDate.fields.estimatedDueDate',
                  { defaultValue: 'Estimated due date' }
                ),
                estimatedDueDateFromScan: t(
                  'pregnancyDueDate.fields.estimatedDueDateFromScan',
                  { defaultValue: 'Estimated due date (from scan)' }
                ),
              } as Record<string, string>
            )[DATE_FIELD_LABEL[form.basis]]
          }
          rightAccessory={
