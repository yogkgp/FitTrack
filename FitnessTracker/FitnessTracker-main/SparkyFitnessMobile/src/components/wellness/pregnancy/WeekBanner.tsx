...
const WeekBanner: React.FC<WeekBannerProps> = ({ ga, dueDate, onEdit }) => {
  ...
  const dueLabel = !discreetMode && (
    <Text className="text-text-secondary text-sm">
      {t('pregnancy.weekBanner.due', { defaultValue: 'Due' })}{' '}
      <Text className="text-accent-primary font-semibold">
        {formatDate(dueDate, dateLocale)}
      </Text>
    </Text>
  );
    ...
    <View className="bg-surface rounded-xl p-4 shadow-sm gap-3">
      <View className="flex-row items-end justify-between">
        <View>
          <Text className="text-text-secondary text-base">
            {discreetMode
              ? t('pregnancy.weekBanner.wellnessProgress', {
                  defaultValue: 'Wellness Progress',
                })
              : trimesterLabel}
          </Text>
          <Text className="text-text-primary text-2xl font-bold">
            {discreetMode
              ? t('pregnancy.weekBanner.week', {
                  defaultValue: 'Week {{week}}',
                  week: ga.week,
                })
              : t('pregnancy.weekBanner.weekDay', {
                  defaultValue: '{{week}}w {{day}}d',
                  week: ga.week,
                  day: ga.day,
                })}
          </Text>
        ...
        {onEdit ? (
          <TouchableOpacity
            ...
            accessibilityLabel={t('pregnancy.weekBanner.edit', {
              defaultValue: 'Edit pregnancy details',
            })}
      ...
      <View className="h-2 rounded-full bg-progress-track overflow-hidden">
        <View
          ...
          style={{
            width: `${pct * 100}%`,
            backgroundColor: tokens.phasePregnant,
          }}
        />
      </View>
      ...
      {!discreetMode && (
        <Text className="text-text-secondary text-base">
          {ga.daysRemaining > 0
            ? t('pregnancy.weekBanner.daysToGo', {
                defaultValue: '{{days}} days to go',
                days: ga.daysRemaining,
                count: ga.daysRemaining,
              })
            : t('pregnancy.weekBanner.anyDayNow', {
                defaultValue: 'Any day now',
              })}
        </Text>
      )}
    </View>
