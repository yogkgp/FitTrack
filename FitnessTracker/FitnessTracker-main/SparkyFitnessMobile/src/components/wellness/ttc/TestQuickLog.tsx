...
import {
  useCycleTests,
  useCycleTestMutations,
} from '../../../hooks/useCycleTests';
...
function getResultLabel(
  ...
  switch (result) {
    case 'negative':
      return t('testQuickLog.results.negative', { defaultValue: 'Negative' });
    case 'low':
      return t('testQuickLog.results.low', { defaultValue: 'Low' });
    case 'high':
      return t('testQuickLog.results.high', { defaultValue: 'High' });
    case 'peak':
      return t('testQuickLog.results.peak', { defaultValue: 'Peak' });
    case 'faint':
      return t('testQuickLog.results.faint', { defaultValue: 'Faint' });
    case 'positive':
      return t('testQuickLog.results.positive', { defaultValue: 'Positive' });
    default:
      return result;
  }
}
...
function getTestTypeLabel(
  ...
  switch (type) {
    case 'opk':
      return t('testQuickLog.ovulationShort', { defaultValue: 'Ovulation' });
    case 'hpt':
      return t('testQuickLog.pregnancyShort', { defaultValue: 'Pregnancy' });
    default:
      return type;
  }
}
...
const TestQuickLog: React.FC<TestQuickLogProps> = ({ date }) => {
  ...
  const { createTestEntryAsync, isCreating, deleteTestEntryAsync } =
    useCycleTestMutations();

  const handleLog = async (result: string) => {
    try {
      await createTestEntryAsync({
        entry_date: date,
        test_type: testType,
        result,
      });
      Toast.show({
        type: 'success',
        text1: t('testQuickLog.testLogged', { defaultValue: 'Test logged' }),
      });
    } catch {
      Toast.show({
        type: 'error',
        text1: t('testQuickLog.logFailed', {
          defaultValue: 'Could not log test',
        }),
      });
    }
  };
  ...
  const handleDelete = async (entry: SharedCycleTestEntry) => {
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: t('testQuickLog.removeFailed', {
          defaultValue: 'Could not remove test',
        }),
      });
    }
  };
    ...
    <View className="bg-surface rounded-xl p-4 border-0 shadow-sm gap-3">
      ...
      <SegmentedControl
        segments={[
          {
            key: 'opk',
            label: t('testQuickLog.ovulation', {
              defaultValue: 'Ovulation (OPK)',
            }),
          },
          {
            key: 'hpt',
            label: t('testQuickLog.pregnancy', {
              defaultValue: 'Pregnancy (HPT)',
            }),
          },
        ]}
        activeKey={testType}
          ...
          <View className="rounded-xl overflow-hidden">
            {tests.slice(0, 6).map((entry, idx) => (
              <SwipeableDeleteRow
                ...
                title={t('testQuickLog.entryTitle', {
                  defaultValue: '{{type}} · {{result}}',
                  type: getTestTypeLabel(t, entry.test_type),
                  result: getResultLabel(t, entry.result),
                })}
                onConfirmDelete={() => handleDelete(entry)}
                className={`flex-row items-center justify-between py-2.5 ${
                  idx < Math.min(tests.length, 6) - 1
